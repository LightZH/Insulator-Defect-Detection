...
def parse_args():
    ...
    parser.add_argument('--save_dir', default='./results_ub', type=str)
...
def main(args):
    ...
    for (data, p_gt, i_gt) in tqdm(test_loader):
        ...
        data = data.to(device)
        # upbound
        mask_image = get_GT(p_gt.cpu().numpy(), args)
        rect, mask = run_main_ub(data, model_mae, mask_image, args.mask_size)
        mask_path = os.path.join(save_path, "mask")
        if not os.path.isdir(mask_path):
            os.mkdir(mask_path)
        for i in range(rect.shape[0]):
            reconsts.append(rect[i].cpu().detach().numpy())
            masks.append(mask[i].cpu().detach().numpy())
            boxes.append(box)
        if time_flag == 0:
    ...
    for num in range(len(reconsts)):
        ...
        cv2.imwrite(os.path.join(mask_path, (name[:-4] + "_m" + name[-4:])), denormalize(masks[num]))
